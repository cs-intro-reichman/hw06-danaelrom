...
public class Runigram {
	...
	public static Color luminance(Color pixel) {
		...
		Color newcColor = new Color(grey,grey,grey);
		return newcColor;
	}
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		for(int i = 0; i < height; i++){
			for(int j = 0; j < width; j++){
				int pixHeight = (int)(i * image.length/height);
				int pixWidth = (int)(j * image[0].length/width);
				res[i][j] = image[pixHeight][pixWidth];
			}
		}		
		return res;
	}
